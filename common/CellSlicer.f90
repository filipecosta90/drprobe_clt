...
subroutine CS_GETSLICE_POT2(nslc, nx, ny, nrx, nry, nfl, ndw, wl, pot, nerr)
  ...
  integer*4 :: i, j, i1, j1, ia, ia2, jptr ! iterators
  integer*4 :: lndw
  integer*4 :: na ! atom count
  integer*4 :: dimx, dimy, nyqx, nyqy ! sampling numbers
  integer*4 :: infl ! internal frozen lattice flag

  integer*4, dimension(:), allocatable :: ja ! hash table to access slice IDs from cell IDs
  integer*4, dimension(:), allocatable :: jld ! hash table to access slice IDs from cell IDs
  real*4 :: sx, sy, itogx, itogy ! sampling constants
  real*4 :: ht ! high tension in kV
  real*4 :: dx, dy ! atom position
  real*4 :: fldamp, biso ! displacement calculation helpers
  real*4, dimension(:), allocatable :: pocc ! atom occupancy
  real*4 :: dsz, vol ! slice thickness and volume in nm^3
  real*4 :: pscal ! potential scaling factor
  real*4 :: apdmp ! aperture function (damping factor at outer Fourier-space perimeter >2/3 gmax)
  ...
  complex*8, dimension(:,:), allocatable :: CS_scagn_z ! calculated CS_scagn_x
  ...
  CS_scagn_x(1:ny,1:nx) = CS_scagn(1:ny,1:nx,1)
  CS_scagn_y(1:ny,1:nx) = CS_scagn(1:ny,1:nx,2)
  CS_scagn_z(1:ny,1:nx) = CS_scagn(1:ny,1:nx,3)
  ...
  if (infl==1) then                     ! dice frozen lattice displacements (x,y) and apply
    ...
    do ia=1, na ! loop ia over all atoms in slice to calculate translation phase factor
    !
    if (CS_atlnk(ja(ia))==0) then ! independent atom site
      dx = fldamp*dwc(ia)*GaussRand()         !   fldamp*dwc = sqrt( Biso / 8*pi^2) = sqrt( <u_s^2> )
      dy = fldamp*dwc(ia)*GaussRand()
      lx(ia) = lx(ia) + dx          ! add random x displacement
      ly(ia) = ly(ia) + dy          ! add random y displacement
    else ! dependent atom site
      ia2 = jld(CS_atlnk(ja(ia)))
      lx(ia) = lx(ia2)
      ly(ia) = ly(ia2)
    end if
  ...
  do ia=1, na ! loop ia over all atoms in slice
  !
  jptr = CS_scampptr(ja(ia))                  ! get atom type ID in scattering data
  pocc(ia) = CS_atocc(ja(ia))                     ! Occupancy
  crgio = CS_atcrg(ja(ia))                    ! ionic charge

  form_factor(ia,1:ny,1:nx) = CS_scaff2d(1:ny,1:nx,jptr)
  form_factor_ionic(ia,1:ny,1:nx) = crgio*CS_scagn_z(1:ny,1:nx)

  ! removed one if via negation of the conjunction ( crgio.eq.0.0  .or. CS_useextsca.eq.0  )
  ! ion (slower, need to multiply ionic potential)
  ! apply DWF to ionic part (slower)
  if ( crgio.NE.0.0  .and. CS_useextsca.NE.0 .and. dwflg==.true. ) then 
    form_factor(ia,1:ny,1:nx) = form_factor(ia,1:ny,1:nx) + CS_scadwf(1:ny,1:nx,jptr) * form_factor_ionic(ia,1:ny,1:nx)
  end if 

  ! ion (slower, need to multiply ionic potential)
  ! no DWF in ionic part (faster)
  if ( crgio.NE.0.0  .and. CS_useextsca.NE.0 .and. dwflg==.false. ) then 
    form_factor(ia,1:ny,1:nx) = form_factor(ia,1:ny,1:nx) + form_factor_ionic(ia,1:ny,1:nx)
  end if
  ...
  lcw(1:ny,1:nx) = lcw(1:ny,1:nx) + pocc(ia) * form_factor(ia,1:ny,1:nx) * translation_phase_factor(ia,1:ny,1:nx)
  !
  ! update progress indicator
